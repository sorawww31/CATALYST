...
from .mixing_data_augmentations import Cutmix, Cutout, Maxup, Mixup  # mody-aug
...
class Kettle:
        ...
        if self.args.poisonkey is None:
            if self.args.benchmark != "":
                ...
                self.benchmark_construction(
                    setup_dict[self.args.benchmark_idx]
                )  # using the first setup dict for benchmarking
            else:
    ...
    def print_status(self):
        ...
        print(
            f'--Target images assigned intended class {", ".join([class_names[i] for i in self.poison_setup["intended_class"]])}.'
        )
    ...
    def prepare_diff_data_augmentations(self, normalize=True):
        ...
        if self.augmentations is not None or self.args.paugment:
            ...
            if self.augmentations == "default":
                self.augment = RandomTransform(**params, mode="bilinear")
            elif self.augmentations == "default-no-align":
            ...
            elif self.augmentations == "default-nn-no-align":
            ...
            elif self.augmentations == "affine-trafo-no-align":
                self.augment = RandomTransformFixed(
                    **params, mode="bilinear", align=False
                )
            elif self.augmentations == "affine-trafo-nn-no-align":
                self.augment = RandomTransformFixed(
            ...
            if self.mixing_method and self.mixing_method["type"]:
    ...
    def deterministic_construction(self):
        ...
        print(
            f"Initializing Poison data (chosen images, examples, targets, labels) as {self.args.poisonkey}"
        )
        ...
        self.poisonset, self.targetset, self.validset = (
            self._choose_poisons_deterministic(target_id)
        )
    ...
    def random_construction(self):
        ...
        if self.args.local_rank is None:
            ...
            print(
                f"Initializing Poison data (chosen images, examples, targets, labels) with random seed {self.init_seed}"
            )
        else:
            ...
            if rank == 0:
                print(
                    f"Initializing Poison data (chosen images, examples, targets, labels) with random seed {init_seed.item()}"
                )
    ...
    def _choose_poisons_randomly(self):
        ...
        if self.poison_setup["target_class"] is not None:
            ...
            self.target_ids = np.random.choice(
                class_ids, size=self.args.targets, replace=False
            )
        else:
            total_ids = []
            for index in range(
                ...
                total_ids.append(idx)
            self.target_ids = np.random.choice(
                total_ids, size=self.args.targets, replace=False
            )
    ...
    def export_poison(self, poison_delta, savename, path=None, mode="automl"):
        ...
        if mode == "packed":
            ...
            name_candidate = (
                f"{self.args.name}_{self.args.dataset}T{targetclass}P{poisonclass}"
            )
            ...
            np.save(
                os.path.join(path, savename + "_poisoned_training_data.npy"),
                training_data,
            )
            np.save(
                os.path.join(path, savename + "_poisoned_training_labels.npy"), labels
            )
            ...
            with open(os.path.join(sub_path, "target.pickle"), "wb+") as file:
                pickle.dump(
                    (_torch_to_PIL(target), target_label),
                    file,
