...
class _Witch:
    ...
    def _brew(self, victim, kettle):
        ...
        print(
            f"Poisons with minimal target loss {self.stat_optimal_loss:6.4e} selected."
        )
    ...
    def _initialize_brew(self, victim, kettle):
        ...
        self.targets = torch.stack([data[0] for data in kettle.targetset], dim=0).to(
            **self.setup
        )
        self.intended_classes = torch.tensor(kettle.poison_setup["intended_class"]).to(
            device=self.setup["device"], dtype=torch.long
        )
        self.true_classes = torch.tensor([data[1] for data in kettle.targetset]).to(
            device=self.setup["device"], dtype=torch.long
        )
        ...
        if self.args.target_criterion in ["cw", "carlini-wagner"]:
            ...
            self.target_grad, self.target_gnorm = victim.gradient(
                self.targets, self.true_classes
            )
            ...
            self.target_grad, self.target_gnorm = victim.sharp_grad(
                criterion, self.targets, self.intended_classes, self.args.sharpsigma
            )
        elif self.args.target_criterion in ["worstsharp"]:
            criterion = nn.CrossEntropyLoss()
            self.target_grad, self.target_gnorm = victim.worst_sharp_grad(
                criterion, self.targets, self.intended_classes, self.args.sharpsigma
            )
        else:
    ...
    def _run_trial(self, victim, kettle):
        ...
        for step in range(self.args.attackiter):
            ...
            if step % (self.args.attackiter // 5) == 0 or step == (
                ...
                print(
                    f"Iteration {step}: Target loss is {target_losses:2.4f}, "
    ...
    def _batched_step(self, poison_delta, poison_bounds, example, victim, kettle):
        ...
        if len(batch_positions) > 0:
            ...
            closure = self._define_objective(
                inputs, labels, self.targets, self.intended_classes, self.true_classes
            )
            loss, prediction = victim.compute(
                closure, self.target_grad, self.target_clean_grad, self.target_gnorm
            )
