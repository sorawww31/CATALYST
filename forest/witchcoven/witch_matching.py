...
class WitchGradientMatching(_Witch):
    ...
    def _define_objective(
        self, inputs, labels, targets, intended_classes, true_classes
    ):
        """Implement the closure here."""

        def closure(
            model, criterion, optimizer, target_grad, target_clean_grad, target_gnorm
        ):
            ...
            passenger_loss = self._passenger_loss(
                poison_grad, target_grad, target_clean_grad, target_gnorm
            )
    ...
    def _passenger_loss(
        self, poison_grad, target_grad, target_clean_grad, target_gnorm
    ):
        ...
        if self.args.loss == "top10-similarity":
            _, indices = torch.topk(
                torch.stack([p.norm() for p in target_grad], dim=0), 10
            )
        elif self.args.loss == "top20-similarity":
            _, indices = torch.topk(
                torch.stack([p.norm() for p in target_grad], dim=0), 20
            )
        elif self.args.loss == "top5-similarity":
            _, indices = torch.topk(
                torch.stack([p.norm() for p in target_grad], dim=0), 5
            )
        else:
        ...
        for i in indices:
            if self.args.loss in ["scalar_product", *SIM_TYPE]:
                ...
                passenger_loss -= torch.nn.functional.cosine_similarity(
                    target_grad[i].flatten(), poison_grad[i].flatten(), dim=0
                )
            elif self.args.loss == "SE":
                passenger_loss += 0.5 * (target_grad[i] - poison_grad[i]).pow(2).sum()
            elif self.args.loss == "MSE":
                passenger_loss += torch.nn.functional.mse_loss(
                    target_grad[i], poison_grad[i]
                )
        ...
        if self.args.repel != 0:
            for i in indices:
                if self.args.loss in ["scalar_product", *SIM_TYPE]:
                    passenger_loss += (
                        self.args.repel * (target_grad[i] * poison_grad[i]).sum()
                    )
                elif self.args.loss == "cosine1":
                    passenger_loss -= (
                        self.args.repel
                    ...
                    passenger_loss -= (
                        0.5
                        * self.args.repel
                        * (target_grad[i] - poison_grad[i]).pow(2).sum()
                    )
                elif self.args.loss == "MSE":
                    passenger_loss -= self.args.repel * torch.nn.functional.mse_loss(
                        target_grad[i], poison_grad[i]
                    )
    ...
    def _hide_gradient(self, gradient_list):
        ...
        if self.args.gradient_clip is not None:
            total_norm = torch.norm(
                torch.stack([torch.norm(grad) for grad in gradient_list])
            )
        ...
        if self.args.gradient_noise is not None:
            for grad in gradient_list:
                ...
                clip_factor = (
                    1 if self.args.gradient_clip is None else self.args.gradient_clip
                )
                noise_sample = (
                    torch.randn_like(grad) * clip_factor * self.args.gradient_noise
                )
                grad += noise_sample
