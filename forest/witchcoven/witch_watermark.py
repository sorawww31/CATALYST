    ...
    def _brew(self, victim, kettle):
        ...
        for poison_id, (img, label, image_id) in enumerate(kettle.poisonset):
            ...
            delta_slice = torch.max(
                torch.min(delta_slice, self.args.eps / kettle.ds / 255),
                -self.args.eps / kettle.ds / 255,
            )
            delta_slice = torch.max(
                torch.min(delta_slice, (1 - kettle.dm) / kettle.ds - poison_img),
                -kettle.dm / kettle.ds - poison_img,
            )
